 ...
 *
 * Pure functions that accept context via parameters instead of global state.
 * This is the infrastructure layer - data access patterns, not business logic.
 */
...
const polygonToWkt = (rings) => `(${rings.map((ring) => ringToWkt(ring)).join(", ")})`;
 ...
 * @param {object} context.bboxFields - Bbox field names {minx, miny, maxx, maxy}
 * @returns {string} SQL WHERE clause
 */
export const buildPointDistanceWhere = (point, distanceMeters, relation = "within", context = {}) => {
  ...
  if (useSpatial) {
    const geomField = quoteField(context.geometryField);
 ...
 * @param {object} boundaryGeom - Boundary geometry
 * @param {object} context - Query context
 * @param {boolean} context.spatialReady - Spatial extension available
 * @param {string} context.geometryField - Geometry field name
 * @returns {string} SQL WHERE clause
 */
export const buildBoundaryWhere = (operator, boundaryGeom, context = {}) => {
  if (!context.spatialReady || !context.geometryField) {
 ...
 * @returns {string} SQL WHERE clause
 */
export const buildAttributeWhere = (operator, value, context = {}) => {
  if (!value) {
    return "1=1";
  }
  ...
  if (operator === "operator") {
    ...
    const operatorFields = context.operatorFields || [];
    ...
    if (!context.modeField) {
      throw new Error("No mode field found in schema");
    }
    return `${quoteField(context.modeField)} = ${formatLiteral(value)}`;
  }
 ...
 * @returns {string} SQL WHERE clause
 */
export const buildSpatialWhere = (compiled, point, context = {}) => {
  ...
  const buildMainClause = (block) => {
    if (block.target === "selected_point") {
      ...
      return buildPointDistanceWhere(point, block.distance, block.relation, context);
    }
    if (block.target === "boundary") {
      return buildBoundaryWhere(block.relation || "within", globalBoundary, context);
    }
    return null;
  };
  ...
  const buildSupplementalClause = (block) => {
    if (block.operator === "near_point") {
      ...
      return buildPointDistanceWhere(point, block.distance || 300, "within", context);
    }
    if (block.operator === "touches_boundary" || block.operator === "inside_boundary") {
      ...
      return buildBoundaryWhere(block.operator, block.boundary || globalBoundary, context);
    }
    if (block.operator === "operator" || block.operator === "mode") {
      return buildAttributeWhere(block.operator, block.value, context);
    }
    return null;
  };
