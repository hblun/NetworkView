...
- `config.json` and `config.sample.json` control the R2 data endpoints, metadata, and basemap. Any data refresh must also update `public/routes.pmtiles`, `data/parquet/routes.parquet`, and `metadata.json` in tandem.
